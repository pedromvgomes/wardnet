...
/// Test harness that builds a `ProviderServiceImpl` with mocks.
struct TestHarness {
    svc: ProviderServiceImpl,
    tunnel_service: Arc<MockTunnelService>,
}

/// Build a config with all built-in providers disabled (for isolated mock tests).
...
fn build_empty_harness() -> TestHarness {
    ...
    let svc = ProviderServiceImpl::new(registry, tunnel_service.clone());
    TestHarness {
        svc,
        tunnel_service,
    }
